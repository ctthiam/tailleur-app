...
import { creations, categories } from "@/lib/data";
...
export default function CataloguePage() {
  ...
  const filtered = creations.filter((c) => {
    const matchCat  = activeSlug === "tous" || c.categorieSlug === activeSlug;
    const matchSearch =
      search.trim() === "" ||
  ...
  return (
    <div className="pb-nav">
      ...
      <header className="px-5 pt-12 pb-4">
        <h1 className="font-display text-2xl font-semibold text-brun-900 mb-1">
          Catalogue
        </h1>
      ...
      <div className="px-5 mb-4">
        <div className="flex items-center gap-3 bg-white rounded-2xl px-4 py-3 shadow-card">
          ...
          />
          {search && (
            <button onClick={() => setSearch("")} className="text-brun-400 text-xs">
              ✕
            </button>
          )}
        </div>
          ...
          {filtered.map((creation) => (
            ...
            >
              ...
              <div className="p-3">
                ...
                <p className="font-body text-xs font-semibold text-foret-600 mb-1">
                  {creation.prix}
                </p>
