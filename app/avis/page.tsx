...
export default function AvisPage() {
  const [avis, setAvis]       = useState<ApiAvis[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    avisApi.getPublies().then(setAvis).catch(() => {}).finally(() => setLoading(false));
  }, []);
    ...
    <div className="pb-nav">
                ...
                return (
                  <div key={note} className="flex items-center gap-2">
                    ...
                    <div className="flex-1 h-1.5 bg-creme-300 rounded-full overflow-hidden">
                      <div className="h-full bg-or-400 rounded-full" style={{ width: `${pct}%` }} />
                    </div>
      ...
      {loading ? (
        ...
        <div className="px-5 space-y-4">
            ...
            <div key={a.id} className="card p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex-1">
                  ...
                  {a.ville && <p className="font-body text-xs text-brun-400">{a.ville}</p>}
                </div>
          ...
          {avis.length === 0 && (
            <p className="text-center font-body text-sm text-brun-400 py-12">Aucun avis pour le moment</p>
          )}
        </div>
      )}
    </div>
